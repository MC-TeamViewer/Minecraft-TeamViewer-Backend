...
from ..core.models import BattleChunkData
from ..core.versioning import normalize_protocol_version, parse_protocol_version, protocol_at_least
from .battle_chunks import (
    build_battle_chunk_semantic_projection_hash as build_battle_chunk_semantic_projection_hash_helper,
    build_battle_map_observation_hash as build_battle_map_observation_hash_helper,
)
from .config import (
    coerce_bool,
    coerce_float,
    coerce_int,
    load_toml_file,
    normalize_room_code as normalize_room_code_helper,
    parse_battle_chunk_symbol_config,
    parse_congestion_levels,
)
from .delta import (
    build_state_node as build_state_node_helper,
    canonical_number,
    canonical_value,
    compact_state_map,
    compute_field_delta,
    compute_scope_patch,
    delete_report as delete_report_helper,
    has_patch_changes,
    make_empty_patch,
    merge_patch,
    merge_patch_and_validate,
    missing_fields_from_validation_error,
    node_timestamp,
    payload_preview,
    prune_none_fields,
    resolve_report_map,
    state_digest,
    state_digest_plain,
    touch_reports as touch_reports_helper,
    upsert_report as upsert_report_helper,
)
from .tab_sync import normalize_tab_name, normalize_tab_uuid
...
class ServerState:
    ...
    def _load_toml_file(cls, path: Path, label: str) -> dict:
        return load_toml_file(path, label)
    ...
    def _parse_battle_chunk_symbol_config(config: dict) -> Dict[str, str]:
        return parse_battle_chunk_symbol_config(config)

    @staticmethod
    def _coerce_int(value, default: int, min_value: int, max_value: int) -> int:
        return coerce_int(value, default, min_value, max_value)

    @staticmethod
    def _coerce_float(value, default: float, min_value: float, max_value: float) -> float:
        return coerce_float(value, default, min_value, max_value)
    ...
    def _parse_congestion_levels(raw_levels) -> list[tuple[int, float]]:
        return parse_congestion_levels(raw_levels)

    @staticmethod
    def _normalize_tab_uuid(value) -> Optional[str]:
        return normalize_tab_uuid(value)
    ...
    def normalize_room_code(cls, value) -> str:
        return normalize_room_code_helper(value, cls.DEFAULT_ROOM_CODE)
    ...
    def build_battle_map_observation_hash(
        ...
        return build_battle_map_observation_hash_helper(
    ...
    def build_battle_chunk_semantic_projection_hash(self, projected: dict[str, dict]) -> str:
        return build_battle_chunk_semantic_projection_hash_helper(
            self.canonical_value,
            lambda chunk_data: self.build_battle_chunk_sync_data(chunk_data, include_meta=False),
            projected,
        )
    ...
    def compact_state_map(state_map: Dict[str, dict]) -> Dict[str, dict]:
        ...
        return compact_state_map(state_map)
    ...
    def state_digest(cls, state_map: Dict[str, dict]) -> str:
        return state_digest(state_map)

    @classmethod
    def state_digest_plain(cls, state_map: Dict[str, dict]) -> str:
        return state_digest_plain(state_map)
    ...
    def touch_reports(
        ...
        return touch_reports_helper(report_map, object_ids, source_id, current_time)
    ...
    def compute_scope_patch(cls, old_map: Dict[str, dict], new_map: Dict[str, dict], *, full_replace: bool = False) -> dict:
        return compute_scope_patch(old_map, new_map, full_replace=full_replace)
